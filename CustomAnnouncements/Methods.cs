...
namespace CustomAnnouncements
{
    ...
    public class Methods
    {
        ...
        {
            var scpCount = Player.Get(Team.SCPs).Count();
            return str.ReplaceAfterToken('$', new[]
            {
                new Tuple<string, object>("ScpsLeft", scpCount),
                new Tuple<string, object>("MtfLeft", Player.Get(Team.FoundationForces).Count()),
                new Tuple<string, object>("SciLeft", Player.Get(Team.Scientists).Count()),
                new Tuple<string, object>("CdpLeft", Player.Get(Team.ClassD).Count()),
                new Tuple<string, object>("ChiLeft", Player.Get(Team.ChaosInsurgency).Count()),
                new Tuple<string, object>("HumansLeft", Player.List.Count(player => player.Role.Is(out Exiled.API.Features.Roles.HumanRole _) && player.Role.Team != Team.OtherAlive)),
